...
public class InternalConfigurations {
	...
	// in which single writer tasks are still processed after the persistence
	// shutdown hook was called (in milliseconds).
	public static final AtomicInteger PERSISTENCE_SHUTDOWN_GRACE_PERIOD = new AtomicInteger(2000);

	// timeout to wait in seconds for persistence shutdown to finish on HiveMQ
	// shutdown
	public static final AtomicInteger PERSISTENCE_SHUTDOWN_TIMEOUT = new AtomicInteger(300);

	// timeout to wait in seconds for persistence startup executors shutdown to
	// finish on HiveMQ shutdown
	public static final AtomicInteger PERSISTENCE_STARTUP_SHUTDOWN_TIMEOUT = new AtomicInteger(300);

	// the factor to multiply core size with to calculate thread count for
	// initializing persistences
	...
	// max amount of subscriptions to pull from the peristence for extension iterate
	// chunk
	...
	// The threshold at which the topic tree starts to map entries instead of
	// storing them in an array
	public static final AtomicInteger TOPIC_TREE_MAP_CREATION_THRESHOLD = new AtomicInteger(16);
	// The configuration for qos 0 memory hard limit divisor, must be greater than
	// 0.
	...
	// The limit of unacknowledged messages that hivemq will handle, regardless of
	// the client receive maximum
	...
	// The configuration for xodus persistence environment garbage collection
	// deletion delay in ms
	public static final int XODUS_PERSISTENCE_ENVIRONMENT_GC_DELETION_DELAY = 60000;
	// The configuration for xodus persistence environment garbage collection run
	// period in ms
	public static final int XODUS_PERSISTENCE_ENVIRONMENT_GC_RUN_PERIOD = 30000;
	// The configuration for xodus persistence environment garbage collection files
	// interval
	public static final int XODUS_PERSISTENCE_ENVIRONMENT_GC_FILES_INTERVAL = 1;
	// The configuration for xodus persistence environment garbage collection min
	// age
	...
	// The time that entries are cached in memory, in the payload persistence in
	// milliseconds.
	public static final AtomicLong PAYLOAD_CACHE_DURATION = new AtomicLong(10000);
	// The maximum amount of entries that are cached in memory, in the payload
	// persistence.
	...
	// The schedule in which the cleanup for payloads that are not referenced
	// anymore are executed.
	public static final AtomicInteger PAYLOAD_PERSISTENCE_CLEANUP_SCHEDULE = new AtomicInteger(250);
	// The amount of time that a payload is preserved after the last reference was
	// removed. In case the same payload is published again.
	...
	// In case we tried to decrement a reference count that was already zero, a
	// stacktrace will be logged to warn, if this flag is true (default is debug)
	...
	/**
	 * The amount of clean up job tasks that are processed at the same time, in each
	 * schedule interval
	 */
	...
	/**
	 * The amount of time the extension executor shutdown awaits task termination
	 * until shutdownNow() is called. In seconds.
	 */
	 ...
	 */
	public static final boolean AUTH_DENY_UNAUTHENTICATED_CONNECTIONS = true;
